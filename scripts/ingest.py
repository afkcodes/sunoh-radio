    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "streams" in data and len(data["streams"]) > 0:
                ...
                try:
                    station["bitrate"] = int(s.get("bit_rate") or 0)
                except:
                    station["bitrate"] = 0
                try:
                    station["sample_rate"] = int(s.get("sample_rate") or 0)
                except:
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for station in stations:
                    ...
                    if normalized_url not in provider_library:
                        existing = cache.get(normalized_url)
                        ...
                        entry = {
                            ...
                            "image": image,
                            "stream_url": url, # Keep original URL for playability
                        ...
                        if not force_test and existing and existing.get("status") == "working":
                            ...
                            # If cached version has a normalized URL that matches, trust it
                            if not entry["website"] and existing.get("website"):
                                entry["website"] = existing["website"]
                            cached_count += 1

                        provider_library[normalized_url] = entry
                    else:
                        entry = provider_library[normalized_url]
