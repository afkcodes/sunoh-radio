...
async function fetchMyTuner(urls, mode = 'station') {
  ...
  if (mode === 'country') {
    ...
    // Optimization: Block heavy resources for listing pages
    ...
    // Individual Station Mode with Concurrency
    const CONCURRENCY = 5;
    const queue = [...urls];

    const worker = async () => {
      const page = await browser.newPage();
      // Block heavy resources for station pages
      await page.setRequestInterception(true);
      page.on('request', (req) => {
        const type = req.resourceType();
        if (['image', 'font', 'media'].includes(type) || req.url().includes('google')) {
          req.abort();
        } else {
          req.continue();
        }
      ...
      while (queue.length > 0) {
        ...
        try {
          ...
          // Wait shorter for playlist script to populate
          try {
            ...
            // Content might still be available in the DOM even if window._playlist is not ready
          }
          ...
          const data = await page.evaluate(() => {
            ...
            let languages = [];
            const langLabel = Array.from(document.querySelectorAll('.info-label')).find((el) =>
              el.textContent.includes('Language'),
            );
            if (langLabel) {
              languages = Array.from(langLabel.parentElement.querySelectorAll('a')).map((a) =>
                a.textContent.trim(),
              );
            }

            let website = '';
            const websiteLabel = Array.from(document.querySelectorAll('div.string-a')).find(
            ...
            return {
              ...
              name: decodeHtml(
                window.radio_name || document.querySelector('h1')?.textContent?.trim() || '',
              ),
              ...
              genres: Array.from(document.querySelectorAll('a[href*="/radio/genre/"]')).map(
                (a) => a.textContent.trim(),
              ),
              country:
                document.querySelector('a[href*="/radio/country/"]')?.textContent?.trim() || '',
              description:
...
fetchMyTuner(urls, mode).catch((err) => {
